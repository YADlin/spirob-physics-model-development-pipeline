...
STANDALONE (interactive, with hover + click measuring):
    python tools/preview.py --params params.json
    python tools/preview.py --params params.json --nlobe    # also show cross-section
    python tools/preview.py --params params.json --proceed  # skip yes/no prompt
...
#  docs/engineering/GEOMETRY_AUDIT.md F-07 and F-10.
#
...
def _geometry(params):
    """Canonical geometry for a params dict, memoised on parameter values (bounded to eight entries)."""
    key = json.dumps(params, sort_keys=True)
    entry = _GEOM_CACHE.get(key)
    if entry is None:
        entry = (params, _canonical_from_params(params))
        if len(_GEOM_CACHE) >= 8:
            del _GEOM_CACHE[next(iter(_GEOM_CACHE))]
